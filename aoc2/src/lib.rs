...
static RX_GAME: LazyLock<Regex> = LazyLock::new(|| Regex::new(r"Game\s(\d+)").unwrap());
...
pub fn run(config: Config) -> Result<i32, Box<dyn Error>> {
    ...
    for line in contents.lines() {
        ...
        let game = match RX_GAME.captures(line) {
        ...
        if test.red <= MAX.red && test.green <= MAX.green && test.blue <= MAX.blue {
            sum += game;
        }
    }
    Ok(sum)
